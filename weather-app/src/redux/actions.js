...
export function userLocation() {
    return dispatch => {
        function success(pos) {
            ...
            const coords = `${crd.latitude.toFixed(2)}, ${crd.longitude.toFixed(2)}`;
            fetch(`https://api.opencagedata.com/geocode/v1/json?key=50409b6b8c43408ead81d59f9b847c18&q=${coords}&pretty=1&no_annotations=1&`)
                ...
                .then(data => {
                    ...
                    fetch(`https://cors-anywhere.herokuapp.com/https://api.darksky.net/forecast/29e270d552fb7ddfac1a8c371ae8dc03/${coords}?lang=ru`)
                        .then(res => res.json())
                        .then(data => {
                            const temperature = `${Math.floor((data.currently.temperature - 32) * (5 / 9))}°`;
                            const weather = data.currently.summary;
                            dispatch({ type: INIT, coords, city, temperature, weather });
                        })
                })
        }
        function error(err) {
